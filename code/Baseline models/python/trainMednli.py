...
training_mli = load_nli_data(FIXED_PARAMETERS["training_mli"], mli=True)
dev_mli = load_nli_data(FIXED_PARAMETERS["dev_mli"], mli=True)
test_mli = load_nli_data(FIXED_PARAMETERS["test_mli"], mli=True)
...
if not os.path.isfile(dictpath): 
    ...
    if FIXED_PARAMETERS["alpha"] == 0:
        ...
        word_indices = build_dictionary([training_mli])
        print("word indices length ",len(word_indices))
    else:
        word_indices = build_dictionary([training_mnli, training_mli])
    
    logger.Log("Padding and indexifying sentences")
    sentences_to_padded_index_sequences(word_indices, [training_mnli, training_mli, 
                                        dev_matched, dev_mismatched, dev_mli, test_mli, 
    ...
    logger.Log("Padding and indexifying sentences")
    sentences_to_padded_index_sequences(word_indices, [training_mnli, training_mli, 
                                        dev_matched, dev_mismatched, dev_mli, 
                                        test_mli, test_matched, test_mismatched])
...
class modelClassifier:
    ...
    def train(self, train_mnli, train_snli, dev_mat, dev_mismat, dev_snli):        
        ...
        ### Training cycle
        logger.Log("Training... on mnli and Mednli")
        logger.Log("Model will use %s percent of ********MedNLI************ data during training" %(self.alpha * 100))
                ...
                if self.step % self.display_step_freq == 0:
                    ...
                    if self.alpha != 0.:
                        ...
                        logger.Log("Step: %i\t Dev-matched acc: %f\t Dev-mismatched acc: %f\t \
                            Dev-Mednli acc: %f\t MultiNLI train acc: %f\t Mednli train acc: %f" 
                        ...
                        logger.Log("Step: %i\t Dev-matched cost: %f\t Dev-mismatched cost: %f\t \
                            Dev-Mednli cost: %f\t MultiNLI train cost: %f\t Mednli train cost: %f" 
                            % (self.step, dev_cost_mat, dev_cost_mismat, dev_cost_snli, mtrain_cost, strain_cost))
                    else:
                        logger.Log("Step: %i\t Dev-matched acc: %f\t Dev-mismatched acc: %f\t \
                            Dev-Mednli acc: %f\t MultiNLI train acc: %f" %(self.step, dev_acc_mat, 
                                dev_acc_mismat, dev_acc_snli, mtrain_acc))
                        logger.Log("Step: %i\t Dev-matched cost: %f\t Dev-mismatched cost: %f\t \
                            Dev-Mednli cost: %f\t MultiNLI train cost: %f" %(self.step, dev_cost_mat, 
    ...
    def classify(self, examples):
        ...
        return genres, np.argmax(logits[1:]), cost
...
if test == False:
    # classifier.train(training_mnli, training_mli, dev_matched, dev_mismatched, dev_mli)
    ...
    logger.Log("Acc on Mednli test-set: %s" 
        % (evaluate_classifier(classifier.classify, test_mli, FIXED_PARAMETERS["batch_size"]))[0])
else: 
    results, bylength = evaluate_final(classifier.restore, classifier.classify, 
        [test_matched, test_mismatched, test_mli], FIXED_PARAMETERS["batch_size"])
    logger.Log("Acc on multiNLI matched dev-set: %s" %(results[0]))
    logger.Log("Acc on multiNLI mismatched dev-set: %s" %(results[1]))
    logger.Log("Acc on Mednli test set: %s" %(results[2]))
